...
- Bump `version` in the workspace `Cargo.toml`, ensure `hsin_core::VERSION_CODE` is greater than the preceding release (increment it now only when the code changes have not already done so), refresh `Cargo.lock`, and commit the release-only changes together as `chore: release X.Y.Z [skip ci]`.
